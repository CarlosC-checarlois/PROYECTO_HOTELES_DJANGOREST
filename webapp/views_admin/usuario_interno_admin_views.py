...
class UsuarioInternoCreateAjaxView(View):
    def post(self, request):
        ...
        try:
            ...
            if not tipo_doc:
                return JsonResponse({"status": "error", "message": "Debe seleccionar un tipo de documento"}, status=400)
            
            if not documento:
                return JsonResponse({"status": "error", "message": "El documento es obligatorio"}, status=400)
