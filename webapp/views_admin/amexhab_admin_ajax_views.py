...
class AmexHabCreateAjaxView(View):
    def post(self, request):
        try:
            ...
            if not idHabitacion:
                return JsonResponse(
                    {"status": "error", "message": "ID Habitación es requerido"},
                    status=400
                )

            if not idAmenidad:
                return JsonResponse(
                    {"status": "error", "message": "ID Amenidad es requerido"},
                    status=400
                )
            ...
            # ================================
            try:
                existente = api.obtener_amexhab_por_id(idHabitacion, int(idAmenidad))
            except ValueError as ve:
                # por si llega un idAmenidad inválido
                return JsonResponse(
            ...
            if existente is not None:
                # Ya hay un registro con esa habitación y amenidad
                return JsonResponse(
                    {
                        "status": "error",
                        "message": "Ya existe esta amenidad asociada a la habitación seleccionada."
                    },
                    status=400
                )
            ...
            return JsonResponse(
                {"status": "ok", "message": "Amenidad por Habitación creada exitosamente"}
            )
            ...
            return JsonResponse(
                {"status": "error", "message": f"Datos inválidos: {str(ve)}"},
                status=400
            )
        except Exception:
            return JsonResponse(
